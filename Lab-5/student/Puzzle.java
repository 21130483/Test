...
public class Puzzle {
	...
	public int computeH1(Node currentState) {
		...
		for (int i = 0; i < MAX_COL; i++) {
			for (int j = 0; j < MAX_ROW; j++) {
				if (currentState.getTile(i, j) != count && currentState.getTile(i, j) != 0) {
					output++;
				}
				count++;
			}
	...
	public int computeH2(Node currentState) {
		...
		int count = 1;
		for (int i = 1; i < 9; i++) {
				int[] init = currentState.getLocation(i);
				int[] goal = goalState.getLocation(i);
//				result += (Math.abs(i - x[0]) + Math.abs(j - x[1]));
//				count++;
//				if(count==9) {
//					break;
//				}
				result += (Math.abs(goal[0] - init[0]) + Math.abs(goal[1] - init[1]));
		}
		return result;
	}
	...
	public Node moveWhiteTile(Node currentState, char operator) {
		...
		if (operator == 'u') {// Case-1: Move tile UP
			...
			int row = whiteTile[0] + 1;
			int col = whiteTile[1];
			if (row <= 2) {// Tile stands inside the mapa
				int tmp = currentState.getTile(row, col);
				result.updateTile(row, col, 0);
				result.updateTile(whiteTile[0], whiteTile[1], tmp);
				result.setH(computeH2(result));
	...
	public char[] computeH(Node initialState2, Node goalState2) {
